...
EOF
    ...
    # 4. 自动修复主配置 nginx.conf (借鉴你的逻辑)
    if [ -f "/etc/nginx/nginx.conf" ]; then
        # 如果主配置里没写 include，则在 http 块末尾插入
...
EOF
    ...
    if nginx -t > /dev/null 2>&1; then
        ...
        green "Nginx 节点转发配置已自动生效"
    else
        red "Nginx 配置检测失败，正在尝试还原..."
        [[ -f "/etc/nginx/conf.d/sing-box.conf.bak.sb" ]] && cp /etc/nginx/conf.d/sing-box.conf.bak.sb /etc/nginx/conf.d/sing-box.conf
        systemctl restart nginx > /dev/null 2>&1
    fi
}
