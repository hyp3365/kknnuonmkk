...
cf_select_auth() {
    ...
    echo -e " 3. 编辑策略：选择 \033[33m所有域名\033[0m"
	...
	echo -e " \033[32m点击添加策略\033[0m"
    echo -e " 4. 编辑策略：选择 \033[33m整个账户\033[0m"
    echo -e " \033[33mCloudflare One/Zero Trust---Argo Tunnel\033[0m，权限设为 \033[32m全部选择\033[0m"
	echo -e " \033[32m点击添加策略\033[0m"
...
issue_cf_token_cert() {
    ...
    echo -e " \033[33mDNS & Zones---DNS\033[0m，权限设为 \033[32mEdit (编辑)\033[0m"
    echo -e " \033[33mDNS & Zones---Zone Settings\033[0m，权限设为 \033[32mEdit (编辑)\033[0m"
    echo -e " \033[33mRules & Rules & Configuration---Origin\033[0m，权限设为 \033[32mEdit (编辑)\033[0m"	
    ...
    if [[ "$success" != "true" ]]; then
        ...
        if [[ -n "$error_msg" ]]; then
            ...
            red "请检查 API Token 是否正确，以及 Token 是否拥有 Zone Read 权限。"
        fi
        return 1
    fi
    local domains_and_ids
    ...
    "$acme_cmd" \
        --set-default-ca \
        --server letsencrypt >/dev/null 2>&1
    local cert_dir="/root/cert/${cert_domain}"
    mkdir -p "$cert_dir"
    skyblue "开始通过 Cloudflare API Token 自动申请证书..."
    skyblue "证书域名: $cert_domain"
    if ! "$acme_cmd" \
        --issue \
        --dns dns_cf \
    ...
    then
        red "证书申请失败！"
        red "请检查 API Token 是否拥有 DNS Edit 权限。"
        return 1
    fi
    if ! "$acme_cmd" \
        --installcert \
