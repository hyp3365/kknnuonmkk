...
class PanelHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            res = self.do_sync_fanout_action()
            self.send_no_cache_response(
                200,
                "application/json",
        ...
        if path == "/api/add_rule":
            try:
                ...
                if "rules" not in cfg["route"]:
                    cfg["route"]["rules"] = []
                cfg["route"]["rules"].insert(0, new_rule)
                with open(route_file, "w") as f:
                    json.dump(
                        cfg,
                        f,
    ...
    def do_sync_fanout_action(self):
        ...
        try:
            ...
            outbound_data["outbounds"] = [
                ...
                if not (
                    isinstance(x, dict)
                    and str(x.get("tag", "")).startswith("fanout-")
                )
            ]
            outbound_data["outbounds"].extend(nodes)
            os.makedirs(
